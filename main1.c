...
#include <sys/select.h> 
 #include <string.h>
#include "crear_matriz.h" //biblioteca para movimiento continuo.
//#include <conio.h>
/*
PARA LINUX*/
#include <termios.h>
...
{
 for (int i = 0; i < nFilas; i++) {
    for (int j = 0; j < nColumnas; j++) { // corregir el bucle
        if (matriz[i][j] == PACMAN || matriz[i][j] == INKY || 
            matriz[i][j] == PINKY || matriz[i][j] == BLINKY || matriz[i][j] == CLYDE) {
            matriz[i][j] = VACIO; // corregir el operador de asignación
        }
    }
}
}
    ...
    if (newX >= 0 && newX < nFilas && newY >= 0 && newY < nColumnas)
    {
        ...
        {   
            ...
            if(*encima){  //ver si el fantasma está encima de un punto o no
                matriz[xFantasma][yFantasma]=PUNTO;  //Si lo está la posición pasa a ser un punto
            }else{
                matriz[xFantasma][yFantasma] = VACIO;  // Y si no la posición pasa a ser vacio
                *encima=1;
            }
             // Dejar vacío la posición anterior
            matriz[newX][newY] = FANTASMA;     // Mover Fantasma
        }else if(matriz[newX][newY] == VACIO){
            if(*encima){   //la misma logica
                matriz[xFantasma][yFantasma] = PUNTO;
